...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        highestProbabilityPos = []                                  # for each belief, get the position with highest
        for belief in livingGhostPositionDistributions:             # probability
            highestProbabilityPos.append(belief.argMax())

        bestPos, bestProbability = None, 0.0                        # from all of the best positions, find the best of
        index = 0                                                   # the best
        for position in highestProbabilityPos:
            if livingGhostPositionDistributions[index][position] >= bestProbability:
                bestPos = position
                bestProbability = livingGhostPositionDistributions[index][position]
            index += 1

        possibleActions = []                                        # calculate the distance of each action to the best
        for action in legal:                                        # pos and choose action that gets pacman closest
            if action is not Directions.STOP:
                successorPosition = Actions.getSuccessor(pacmanPosition, action)
                possibleActions.append((self.distancer.getDistance(successorPosition, bestPos), action))
