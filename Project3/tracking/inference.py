...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance is None:                                               # Handles when ghost is captured
            allPossible[self.getJailPosition()] = 1.0
        else:
            for p in self.legalPositions:                                       # otherwise update belief by using the
                distance = util.manhattanDistance(p, pacmanPosition)            # emissions model and distance
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        """
        allPossible = util.Counter()                                    # for every old position, get the distribution
        for oldPos in self.legalPositions:                              # of the new position and then update the belief
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
            for newPos, prob in newPosDist.items():
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        counter = 0                                           # make the list of particles and distribute them equally
        while counter < self.numParticles:                    # keeps adding the list of legal positions until you hit
            for pos in self.legalPositions:                   # the number of particles
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance is None:                                                # Takes care of case 1, jailed ghost
            self.particleList = [self.getJailPosition()] * self.numParticles
        else:
            newBeliefs = util.Counter()                                          # otherwise update beliefs based on
            oldBeliefs = self.getBeliefDistribution()                            # the emissions model and old belief.
            for p in self.legalPositions:
                newPosDist = util.manhattanDistance(pacmanPosition, p)
                newBeliefs[p] = oldBeliefs[p] * emissionModel[newPosDist]
            if newBeliefs.totalCount() == 0:                                     # Case 2, if all 0, initialize
                self.initializeUniformly(gameState)
            else:
                self.particleList = []                                           # otherwise resample
    ...
    def elapseTime(self, gameState):
        ...
        belief distribution
        """
        "*** YOUR CODE HERE ***"                        # over time, sample from the distribution of new positions.
        newParticleList = []                            # update the list of particles
    ...
    def getBeliefDistribution(self):
        ...
        beliefDistribution = util.Counter()                 # converts the list of particles into a belief dist.

        for pos in self.particleList:
            beliefDistribution[pos] += 1.0
