...
def enhancedFeatureExtractorDigit(datum):
    ...
    ## DESCRIBE YOUR ENHANCED FEATURES HERE...
        look for horizontal, vertical, and diagonal parts of the numbers. They test if there is an empty space next to
        the pixel and determine if the pixel is part of a hori, vert, diag, part of a number.
    ##
    """
    ...
    "*** YOUR CODE HERE ***"
    onPixels = []
    for x in range(DIGIT_DATUM_WIDTH - 1):              # attempt to make it faster by getting only pixels that are on
        for y in range(DIGIT_DATUM_HEIGHT - 1):         # otherwise make the features 0 to save some time.
            if datum.getPixel(x, y) > 0:
                onPixels.append((x, y))
            else:
                features[('v', x, y)] = 0
    ...
    for x, y in onPixels:                               # for the onPixels, calculate the value for features
        features[('v', x, y)] = 1 if (datum.getPixel(x, y) > (datum.getPixel(x, y+1))
                                      or (datum.getPixel(x, y) > (datum.getPixel(x, y-1)))) else 0
        features[('h', x, y)] = 1 if (datum.getPixel(x, y) > (datum.getPixel(x + 1, y))
                                      or (datum.getPixel(x, y) > (datum.getPixel(x - 1, y)))) else 0
        features[('d', x, y)] = 1 if (datum.getPixel(x, y) > (datum.getPixel(x + 1, y + 1))
                                      or (datum.getPixel(x, y) > (datum.getPixel(x - 1, y - 1)))) else 0
...
def enhancedPacmanFeatures(state, action):
    ...
    features = util.Counter()
    "*** YOUR CODE HERE ***"
    features["Stop"] = 100 if action == Directions.STOP else 0          # for StopAgent
    ...
    features['win'] = 100 if successor.isWin() else 0
    features['lose'] = 100 if successor.isLose() else 0
    features['numCapsules'] = len(successor.getCapsules())

    # food distance and closest food distance
    if len(foodPositions) != 0:
        foodDist = [util.manhattanDistance(x, pacmanPosition) for x in foodPositions]
        for i in range(len(foodDist)):
            features[('foodDist', i)] = foodDist[i]
        features['closestFood'] = min(foodDist)

    # capsule distance and closest capsule distance
    if len(capsulePositions) != 0:
        capsuleDist = [util.manhattanDistance(x, pacmanPosition) for x in capsulePositions]
        for i in range(len(capsuleDist)):
            features[('capsuleDist', i)] = capsuleDist[i]
        features['closestCap'] = min(capsuleDist)

    # ghost distance and closest ghost as features
    ghostDist = [util.manhattanDistance(x, pacmanPosition) for x in ghostPositions]
    for i in range(len(ghostDist)):
        features[('ghostDist', i + 1)] = ghostDist[i]
